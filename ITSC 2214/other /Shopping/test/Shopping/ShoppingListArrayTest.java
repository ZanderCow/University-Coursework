...
public class ShoppingListArrayTest {
    ...
    public void testAdd() {
        ...
        // Test adding null entry to empty list
        instance.add(null);
        assertEquals(0, instance.size());
        ...
        // Test general case (content) by calling the contains method
        assertTrue(instance.contains(entry1));
        try {
            // Test general case (content) by comparing entry1 Grocery object 
            // to the contents of the ShoppingListArray.
            assertEquals(0, instance.find(0).compareTo(entry1));
        } catch (EmptyCollectionException ece) {
            fail("Collection should not be empty in testAdd #1");
        }
        
        // Test combined quantity case by adding an item that's already there.
        Grocery duplicate =
            new Grocery(entry1.getName().toLowerCase(),entry1.getCategory());
        duplicate.setQuantity(1);
        instance.add(duplicate); // Try to add this duplicate
        System.out.println(instance.toString());
        // Test contents after trying to add.  Expected result is that there is
        // still only one item but the quantity has increased
        try {
            // Item at index 0 is still the entry1 Grocery object
            assertEquals(0, instance.find(0).compareTo(entry1));
            // Quantity should be 2 now
            assertEquals(2, instance.find(0).getQuantity());
        } catch (EmptyCollectionException ece) {
    ...
    public void testIndexOf() {
        Grocery entry1 = new Grocery("Mayo", "Dressing / Mayo", 1, 2.99f, 1);
        Grocery entry2 = new Grocery("Frosted Flakes", "Cereal", 6, 3.17f, 2);

        // Test element not found in empty list case
        ...
        // Test element not found in non-empty list case
        instance.add(entry1);
        ...
        // Test element found in non-empty list case
        try {
    ...
    public void testContains() {
        ...
        // Test element does not exist in empty list case
        assertFalse(instance.contains(entry1));

        instance.add(entry1);
        
        // Test element does not exist in non-empty list case
        assertFalse(instance.contains(entry2));
